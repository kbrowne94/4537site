const lab1Quiz = {
  quiz: [
    ...
    },
    {
      ...
      choices: ["undefined", "hexadecimal", "null", "string"],
    },
    {
      question: "What Method is used to remove the last value of an array?",
      choices: [".shift()", ".unshift()", ".pop()"],
    },
    {
...
const choice = parseInt(
  window.prompt("please enter value from 0-5 inclusive"),
  10
);
if (choice < 0 || choice > 5) {
  window.alert("error! you entered a number out of range please refresh and try again");
} else {
  for (let i = 0; i < choice; i++) {
    $("body").append(
      $("<div/>", { class: "question", id: `q${i}` }).append(
        ...
        $("<div/>", { class: "answer" })
          .append(
            $("<input/>", {
              type: "radio",
              name: `q${i}`,
              value: `${lab1Quiz.quiz[i].choices[j]}`,
            })
          )
          .append(
            $("<label/>", {
              for: "${lab1Quiz.quiz[i].choices[j]}",
              text: `${lab1Quiz.quiz[i].choices[j]}`,
            })
          )
      );
    }
